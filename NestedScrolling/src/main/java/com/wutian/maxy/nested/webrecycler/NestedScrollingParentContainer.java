...
public class NestedScrollingParentContainer extends ViewGroup implements NestedScrollingParent2 {
    ...
    private NestedScrollingWebWrapper mChildWebView;
    private RecyclerView mChildRecyclerView;

    private NestedScrollingParentHelper mParentHelper;
    private Scroller mScroller;
    private VelocityTracker mVelocityTracker;
    private boolean hasNotifyRVShow = false;
    public NestedScrollingParentContainer(Context context) {
        this(context, null);
    }

    public NestedScrollingParentContainer(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NestedScrollingParentContainer(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mChildWebView != null && mChildWebView.isWebDataLoaded()) {
            if (!hasNotifyRVShow && mChildWebView.getMeasuredHeight() < getHeight())
                notifyRecyclerViewShow();
        }
    }
        ...
        if (mScroller != null && mScroller.computeScrollOffset()) {
            ...
            switch (mCurFlyingType) {
                case FLYING_FROM_WEBVIEW_TO_PARENT://WebView向父控件滑动
                    Log.d("wwwwwwww", "parent computeScroll   FLYING_FROM_WEBVIEW_TO_PARENT " );
                    if (mIsRvFlyingDown) {
                ...
                case FLYING_FROM_PARENT_TO_WEBVIEW://父控件向WebView滑动
                    Log.d("wwwwwwww", "parent computeScroll   FLYING_FROM_PARENT_TO_WEBVIEW " );
                    scrollTo(0, currY);
                    invalidate();
                ...
                case FLYING_FROM_RVLIST_TO_PARENT://RecyclerView向父控件滑动，fling事件，单纯用于计算速度。RecyclerView的flying事件传递最终会转换成Scroll事件处理.
                    Log.d("wwwwwwww", "parent computeScroll   FLYING_FROM_RVLIST_TO_PARENT " );
                    if (getScrollY() != 0) {
                        invalidate();
    ...
    private void findWebView(ViewGroup parent) {
        ...
        for (int i = 0; i < count; i++) {
            View child = parent.getChildAt(i);
            if (child instanceof NestedScrollingWebWrapper && TAG_NESTED_SCROLL_WEBVIEW.equals(child.getTag())) {
                mChildWebView = (NestedScrollingWebWrapper) child;
                break;
            }
            if (child instanceof ViewGroup) {
    ...
    public boolean onStartNestedScroll(@NonNull View child, @NonNull View target, int axes, int type) {
        ...
        Log.d("wwwwwwww", "parent onStartNestedScroll    " + b);
        return b;
    }
    ...
    public int getNestedScrollAxes() {
        ...
        Log.d("wwwwwwww", "parent nestedScrollAxes    " + nestedScrollAxes);
        return nestedScrollAxes;
    }

    @Override
    public void onNestedScrollAccepted(@NonNull View child, @NonNull View target, int axes, int type) {
        Log.d("wwwwwwww", "parent onNestedScrollAccepted    " );
        getNestedScrollingHelper().onNestedScrollAccepted(child, target, axes, type);
    }

    @Override
    public void onStopNestedScroll(@NonNull View target, int type) {
        Log.d("wwwwwwww", "parent onStopNestedScroll    " );
        getNestedScrollingHelper().onStopNestedScroll(target);
    }

    @Override
    public boolean onNestedPreFling(View target, float velocityX, float velocityY) {
        Log.d("wwwwwwww", "parent onNestedPreFling    "    );
        if (target instanceof NestedScrollingWebWrapper) {
    ...
    public boolean onNestedFling(View target, float velocityX, float velocityY, boolean consumed) {
        Log.d("wwwwwwww", "parent onNestedFling    " );
        return false;
    }

    @Override
    public void onNestedScroll(@NonNull View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed, int type) {
        Log.d("wwwwwwww", "parent onNestedScroll    " );
        if (dyUnconsumed < 0) {
    ...
    public void onNestedPreScroll(@NonNull View target, int dx, int dy, @Nullable int[] consumed, int type) {
        Log.d("wwwwwwww", "parent onNestedPreScroll  dddddddddd  " + dy);
        ...
        if (dy > 0 && isWebViewBottom && getScrollY() < getInnerScrollHeight()) {
            Log.d("wwwwwwww", "parent WebView=====================    ");
            //为了WebView滑动到底部，继续向下滑动父控件
            scrollBy(0, dy);
            if (consumed != null) {
                consumed[1] = dy;
            }
        } else if (dy < 0) {     //dy<0 向下滑动
            Log.d("wwwwwwww", "parent RecyclerView=====================    ");
            boolean needScroll = false;
            if (target instanceof RecyclerView) {
                needScroll = isCenter && isRecyclerViewScrollTopItem();
            } else if (target instanceof NestedScrollingWebWrapper) {
                needScroll = /*isCenter && */!isWebContentIsTop();
            } else {
                needScroll = isCenter;
            }
    ...
    private boolean isWebContentIsTop() {
        return !mChildWebView.hasScrolledContent();
    }

    private boolean isRecyclerViewScrollTopItem() {
        LinearLayoutManager layoutManager = (LinearLayoutManager) mChildRecyclerView.getLayoutManager();
        ...
        post(new Runnable() {
            ...
            public void run() {
                if (mNestedScrollListener != null)
                    mNestedScrollListener.notifyScroll();
            }
        });
    }

    private void notifyRecyclerViewShow() {
        post(new Runnable() {
            @Override
            public void run() {
                if (mNestedScrollListener != null && !hasNotifyRVShow) {
                    hasNotifyRVShow = true;
                    mNestedScrollListener.notifyRecyclerViewShow();
                }
            }
        });
    }

    private void notifyRecyclerViewToPending() {
        post(new Runnable() {
            @Override
            public void run() {
                if (mNestedScrollListener != null) {
                    mNestedScrollListener.notifyRecyclerViewToPending();
                }
            }
        });
    }
    ...
    public interface NestedScrollListener {
        void notifyRecyclerViewShow();

        void notifyRecyclerViewToPending();
